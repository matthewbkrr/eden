defmodule EdenWeb.Endpoint do
  ...
  #
  # `brotli` serves the `.br` files the Docker build writes next to them (#511): brotli-11 beats
  # gzip-9 by ~15% on this app's bundles, every browser has asked for it for years, and the work
  # happens once per build rather than per request. Caddy in front leaves an already-encoded
  # response alone, so this reaches the client as-is. Missing `.br` simply falls back to gzip,
  # which is what a dev tree (no Docker build) has.
  plug Plug.Static,
    at: "/",
    from: :eden,
